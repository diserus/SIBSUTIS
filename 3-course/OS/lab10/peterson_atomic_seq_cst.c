...
void* my_thread0(void* arg) {
    ...
    for (; i < 100; i++) {
        ...
        while (atomic_load_explicit(&flag[1], memory_order_seq_cst) && 
               atomic_load_explicit(&turn, memory_order_seq_cst) == 1);

        // Critical section
        sh++;

        atomic_store_explicit(&flag[0], 0, memory_order_seq_cst);
        usleep(1);
    }
    return NULL;
}
...
void* my_thread1(void* arg) {
    int i = 0;
    for (; i < 100; i++) {
        atomic_store_explicit(&flag[1], 1, memory_order_seq_cst);
        atomic_store_explicit(&turn, 0, memory_order_seq_cst);
        while (atomic_load_explicit(&flag[0], memory_order_seq_cst) && 
               atomic_load_explicit(&turn, memory_order_seq_cst) == 0);

        // Critical section
        sh += 2;

        atomic_store_explicit(&flag[1], 0, memory_order_seq_cst);
        usleep(100);
    }
    return NULL;
}
